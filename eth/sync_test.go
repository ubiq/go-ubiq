...
func TestArtificialFinalityFeatureEnablingDisabling_NoDisable(t *testing.T) {
	...
	if !b.blockchain.Config().IsIstanbul(b.blockchain.CurrentBlock().Number()) {
	...
	if !b.blockchain.IsArtificialFinalityEnabled() {
		t.Error(`AF not enabled; The AF disable mechanism triggered by the minimum peers floor should have been short-circuited,
