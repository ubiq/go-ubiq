...
import (
	...
	"github.com/ubiq/go-ubiq/internal/testlog"
...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom ubqhash engine.
	ubqhash := NewTester([]string{server.URL}, false)
	defer ubqhash.Close()
	...
	select {
	case work := <-sink:
		if want := ubqhash.SealHash(header).Hex(); work[0] != want {
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			ubqhash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, ubqhash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch ubqhash result timeout", id+1)
		}
	}
}
