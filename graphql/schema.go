...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Ubiq address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is a Ubiq account at a particular block.
    type Account {
    ...
    # Transaction is an Ubiq transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Ubiq account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Ubiq account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Ubiq block by number or by hash. If neither is
