...
Clef listens to HTTP requests on `rpcaddr`:`rpcport` (or to IPC on `ipcpath`), with the same JSON-RPC standard as Gubiq. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
