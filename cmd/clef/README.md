...
This setup allows a DApp to connect to a remote Ubiq node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Ethereum node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --chainid value         Chain id to use for signing (8=mainnet) (default: 8)
...
In this case, `gubiq` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* Gubiq todos
    ...
    - Relay: Gubiq should be started in `gubiq --signer localhost:8550`.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus Gubiq/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
Clef listens to HTTP requests on `http.addr`:`http.port` (or to IPC on `ipcpath`), with the same JSON-RPC standard as Gubiq. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
