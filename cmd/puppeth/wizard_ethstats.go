...
func (w *wizard) deployEthstats() {
	...
	if existed {
		fmt.Println()
		fmt.Printf("Keep existing IP %v in the blacklist (y/n)? (default = yes)\n", infos.banned)
		if !w.readDefaultYesNo(true) {
			// The user might want to clear the entire list, although generally probably not
			fmt.Println()
			fmt.Printf("Clear out the blacklist and start over (y/n)? (default = no)\n")
			if w.readDefaultYesNo(false) {
				infos.banned = nil
			}
			// Offer the user to explicitly add/remove certain IP addresses
			fmt.Println()
			fmt.Println("Which additional IP addresses should be in the blacklist?")
			for {
			...
			fmt.Println()
			fmt.Println("Which IP addresses should not be in the blacklist?")
			for {
				if ip := w.readIPAddress(); ip != "" {
